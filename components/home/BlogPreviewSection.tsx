    ...
    return (
        <section className="px-6 sm:px-12 bg-gray-600">
            <div className="max-w-6xl mx-auto py-8 sm:py-16 space-y-8 sm:space-y-20">
                <h4 className="text-[clamp(16px,6vw,48px)] font-bold text-center italic text-amber-400">RECENT BLOGS</h4>
                <div className="
                    mx-auto  md:w-full 
              ...
              ">
                    {
                        allPosts?.slice(0, 3).map(post => (
                            <Link key={post._id} href={`/blog/${post.slug.current}`}>
                                <div className="rounded-md w-full overflow-hidden group cursor-pointer">
                                    <div
                                    ...
                                    <div className='flex flex-col  justify-between p-6 bg-white'>
                                        ...
                                        <p className="text-sm sm:text-md text-gray-700 space-y-60">{post.description}</p>
                                    </div>
                                </div>
                            </Link>
                        ))
